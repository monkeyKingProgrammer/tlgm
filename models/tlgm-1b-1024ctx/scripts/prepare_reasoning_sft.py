...
def openmath_rows(limit: int, seed: int):
...
def openr1_rows(limit: int, seed: int):
    dataset = load_dataset("open-r1/OpenR1-Math-220k", "default", split="train", streaming=True)
    for item in limited(dataset.shuffle(seed=seed, buffer_size=5_000), limit):
        yield chat(str(item["problem"]), str(item["solution"]))


def ultrachat_rows(limit: int, seed: int):
...
def science_rows():
    for config in ("ARC-Easy", "ARC-Challenge"):
        for item in load_dataset("allenai/ai2_arc", config, split="train"):
    ...
    for item in load_dataset("allenai/openbookqa", "main", split="train"):
        choices = item["choices"]
        index = answer_index(item["answerKey"], choices["label"])
        options = "\n".join(f"{label}. {text}" for label, text in zip(choices["label"], choices["text"]))
        yield chat(
    ...
    for item in load_dataset("google/boolq", split="train"):
...
def main() -> None:
    ...
    parser.add_argument("--output", default="data/processed/sft_reasoning_large_v2.jsonl")
    parser.add_argument("--validation_output", default="data/processed/sft_reasoning_validation_v2.jsonl")
    parser.add_argument("--work_dir", default="data/processed/reasoning_sources_v2")
    ...
    specs = [
        ("openmath", lambda: openmath_rows(args.openmath, args.seed)),
        ("openr1", lambda: openr1_rows(args.openr1, args.seed + 1)),
        ("ultrachat", lambda: ultrachat_rows(args.ultrachat, args.seed + 2)),
        ("existing_chat", lambda: existing_chat_rows(resolve("../minimind_120m_tiny_chatgpt/data/processed/sft_chat_mix.jsonl"), args.existing_chat)),
        ("verified_logic", lambda: iter(build_rows(args.seed + 3, args.logic_repeats))),
        ("science_reading", science_rows),
    ]
    raw_specs = []
