...
class TLGMArchitectureTests(unittest.TestCase):
    def test_exact_1b_parameter_count(self):
        config = TLGMConfig(
            vocab_size=8192,
            context_length=1024,
            embed_dim=2048,
        ...
        self.assertEqual(sum(parameter.numel() for parameter in model.parameters()), 1_015_592_960)
...
class TLGMDataTests(unittest.TestCase):
    ...
    def test_long_sft_sample_keeps_assistant_targets(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            dataset = ChatSFTDataset(path, PROJECT_DIR / "tokenizer", context_length=32)
            input_ids, labels = dataset[0]
    ...
    def test_each_assistant_turn_is_a_separate_training_target(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(json.dumps(row) + "\n", encoding="utf-8")
            dataset = ChatSFTDataset(path, PROJECT_DIR / "tokenizer", context_length=64)
