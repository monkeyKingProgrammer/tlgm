...
class TLGMTrainer:
    ...
    def validate_pretrain_data(self, data_cfg: dict) -> None:
        ...
        if "vocab_size" in meta and int(meta["vocab_size"]) != self.model_cfg.vocab_size:
            raise ValueError(
                f"Token metadata vocabulary {meta.get('vocab_size')} does not match model vocabulary {self.model_cfg.vocab_size}"
            )
        if data_cfg.get("tokenizer_dir"):
            if "vocab_size" not in meta or "tokenizer_sha256" not in meta:
                raise ValueError("Production pretraining metadata must identify its vocabulary and tokenizer hash")
            tokenizer_dir = self.resolve(data_cfg["tokenizer_dir"])
            ...
            if meta["tokenizer_sha256"] != digest.hexdigest():
                raise ValueError("Pretraining token binary was created with a different tokenizer")
